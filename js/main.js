(function () {
  ...
  galleryFiles.forEach(function (file, index) {
    ...
    item.className = "gallery-item gallery-reveal";
    item.style.setProperty("--gi", index % 6);
  ...
  var galleryRevealEls = grid.querySelectorAll(".gallery-reveal");
  if ("IntersectionObserver" in window) {
    ...
    galleryRevealEls.forEach(function (el) {
  ...
  function renderLightbox(instant) {
    ...
    var nextSrc = basePath + galleryFiles[currentIndex];
  ...
  /* Swipe tactile — navigation fluide sur mobile */
  var touchStartX = 0;
  var touchStartY = 0;
